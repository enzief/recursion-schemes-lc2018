...
object PeanoNumbers {
  ...
  /**
    ...
    */
  val three: PeanoNumber =
    Fix(Maybe.Just(Fix(Maybe.Just(Fix(Maybe.Just(Fix(Maybe.Empty())))))))
...
object BinaryTrees {
  ...
  implicit val treeFunctor: Functor[TreeF] =
    new Functor[TreeF] {
      def map[A, B](fa: TreeF[A])(f: A => B): TreeF[B] =
        fa match {
          case BranchF(lb, l, r) => BranchF(lb, f(l), f(r))
          case LeafF(lb)         => LeafF(lb)
          case EmptyF()          => EmptyF()
        }
    }
  ...
  def treeAlg: Algebra[TreeF, Tree] = {
    case BranchF(lb, l, r) => Branch(lb, l, r)
    case LeafF(lb)         => Leaf(lb)
    case EmptyF()          => Empty()
  }

  def treeCoalg: Coalgebra[TreeF, Tree] = {
    case Branch(lb, l, r) => BranchF(lb, l, r)
    case Leaf(lb)         => LeafF(lb)
    case Empty()          => EmptyF()
  }
  ...
  def toList: Algebra[TreeF, List[Int]] = {
    case BranchF(lb, l, r) => l ::: (lb :: r)
    case LeafF(lb)         => lb :: Nil
    case EmptyF()          => Nil
  }
